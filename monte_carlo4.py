...
def entnahmeplan_rechner(startkapital, zielwert, cagr, jahre):
    monate = jahre * 12
    r_monat = (1 + cagr) ** (1/12) - 1

    faktor_summe = sum((1 + r_monat) ** (monate - i) for i in range(monate))
    entnahme = (startkapital * (1 + r_monat) ** monate - zielwert) / faktor_summe

    kapital = np.zeros(monate + 1)
    kapital[0] = startkapital

    for m in range(1, monate + 1):
        kapital[m] = kapital[m-1] * (1 + r_monat) - entnahme
    ...
    ax.set_title("Entnahmeplan – Kapitalverlauf")
    ax.set_xlabel("Jahre")
...
def plot_simulation(price_paths, final_values, stats, years,
    ...
    # ⭐ Textblock rechts oben – unabhängig vom Datenbereich
